...
# Forked from https://github.com/osrg/ryu/blob/master/ryu/tests/switch/tester.py
...
MSG = {
    ...
    STATE_INIT_THROUGHPUT_FLOW: {
        TIMEOUT:
        'Failed to initialize flow tables of tester_sw: '
    ...
    STATE_THROUGHPUT_FLOW_EXIST_CHK: {
        FAILURE:
        'Added incorrect flows to tester_sw: %(flows)s',
        TIMEOUT:
        'Failed to add flows to tester_sw: '
        'flow stats request timeout.',
        RCV_ERR:
        'Failed to add flows to tester_sw: %(err_msg)s'
    },
...
class Formatter(object):
    """Format Packets for logging"""

    @classmethod
    def fmt(cls, msg, pre_msg='Dict: \n', indent=2, depth=4):
        ...
        """
        return '{}{}'.format(pre_msg, pformat(msg, indent=indent, depth=depth))
    ...
    def fmt_exc(cls, fmt_dict, pre_msg='Dict: \n', keys=None):
        ...
        if not keys:
            keys = []
        d = dict(fmt_dict.__dict__)
        for k in keys:
            try:
                del d[k]
            except KeyError:
                pass
        return Formatter.fmt(d, pre_msg=pre_msg)
...
class OfTester(app_manager.RyuApp):
    ...
    def _set_env(self):
        """Sent test environment on ryu-manager

        """
        keys = ['topo']
        for k in keys:
            if k not in os.environ:
                self.logger.error('ENV variable missing: {}'.format(k))
                sys.exit(1)
        topo_file = os.environ['topo']

        # test case execution interval
        self.interval = 2
        ...
        self.wait_timer = 10
        if os.environ.get('wait_timer'):
            self.wait_timer = int(os.environ.get('wait_timer'))

        self.pytest_args = ['--verbose', '--collect-only']
        if os.environ.get('pytest_args'):
            self.pytest_args = os.environ.get('pytest_args').split()

        with open(topo_file, 'r') as f:
            self.topo = yaml.load(f)

            self.target_dpid = dpid_lib.str_to_dpid(
                str(self.topo['switch_target']['dpid']))
            self.target_of_version = str(
                self.topo['switch_target']['of_version'])
            self.target_recv_port = int(
                self.topo['switch_target']['recv_port'])
            self.target_send_port_1 = int(
                self.topo['switch_target']['send_port_1'])
            self.target_send_port_2 = int(
                self.topo['switch_target']['send_port_2'])

            self.tester_dpid = dpid_lib.str_to_dpid(
                str(self.topo['switch_tester']['dpid']))
            self.tester_of_version = str(
                self.topo['switch_tester']['of_version'])
            self.tester_send_port = int(
                self.topo['switch_tester']['send_port'])
            self.tester_recv_port_1 = int(
                self.topo['switch_tester']['recv_port_1'])
            self.tester_recv_port_2 = int(
                self.topo['switch_tester']['recv_port_2'])
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(Formatter.fmt(self.topo, pre_msg='Topology:'))

        def __get_version(opt):
            vers = {'openflow13': ofproto_v1_3.OFP_VERSION}
            ver = vers.get(opt.lower())
            if ver is None:
                self.logger.error('%s is not supported. '
                                  'Supported versions are %s.', opt,
                                  list(vers.keys()))
                self._test_end()
            return ver

        OfTester.target_ver = __get_version(self.target_of_version)
        OfTester.tester_ver = __get_version(self.tester_of_version)
        ...
        self.test_thread = hub.spawn(self._exec_pytest)

    def _exec_pytest(self):
        """Exec pytest

        """
        pytest.main(self.pytest_args, plugins=[TestSuite(self, self.logger)])
    ...
    def _test_execute(self, json_data, description):
        test_json = TestJson(self, json_data, description)
        test = test_json.test
        ...
        if test.description:
            self.logger.info('Test case description: {}'.format(
                test.description))
        self.thread_msg = None
        ...
        try:
            ...
            for flow in test.prerequisite:
                if isinstance(flow,
                              self.target_sw.dp.ofproto_parser.OFPFlowMod):
                    self.logger.info(
                        Formatter.fmt_exc(
                            flow,
                            keys=['buf'],
                            pre_msg='Sending FlowMod to DUT switch\n'))
                    self._test(STATE_FLOW_INSTALL, self.target_sw, flow)
                    self._test(STATE_FLOW_EXIST_CHK,
                               self.target_sw.send_flow_stats, flow)
                elif isinstance(flow,
                                self.target_sw.dp.ofproto_parser.OFPGroupMod):
                    self.logger.info(Formatter.fmt(flow))
            ...
            for pkt in test.tests:
                ...
                if KEY_FLOW_MISS in pkt:
                    test_type = KEY_FLOW_MISS
                    self.logger.info('PacketIn is NOT expected.')
                    if self.wait_timer > 2:
                        WAIT_TIMER = self.wait_timer / 2
                else:
                    if KEY_EGRESS in pkt:
                        test_type = KEY_EGRESS
                        self.logger.info(
                            'PacketIn is expected from the AUX switch.')
                    elif KEY_PKT_IN in pkt:
                        test_type = KEY_PKT_IN
                        self.logger.info(
                            'PacketIn is expected from the DUT switch.')
                    elif KEY_INGRESS in pkt:
                        test_type = KEY_INGRESS
                        self.logger.info(
                            'PacketIn is expected from the DUT switch.')
                ...
                if KEY_INGRESS in pkt:
                    ...
                    # Check a result based on packet-in
                    result = self._test(STATE_FLOW_MATCH_CHK, pkt)
                    target_pkt_count.append(
                        self._test(STATE_TARGET_PKT_COUNT, True))
                    tester_pkt_count.append(
                        self._test(STATE_TESTER_PKT_COUNT, False))
                    stats_res = self._get_before_and_after_port_stats(
                        target_pkt_count, tester_pkt_count)
                    self.logger.debug('stats_info: {0}'.format(stats_res))
                ...
                if KEY_EGRESS in pkt or KEY_PKT_IN in pkt or KEY_FLOW_MISS in pkt:
                    if result == TIMEOUT:
                        if KEY_FLOW_MISS not in pkt:
                            ...
                            self.logger.info(
                                'PacketIn was not received as expected')
        ...
        if self.interval > 0:
            self.logger.debug(
                'test case interval: sleeping for {} sec(s)'.format(
                    self.interval))
    ...
    def _test(self, state, *args):
        test = {
            STATE_INIT_FLOW: self._test_initialize_flow,
    ...
    def _test_initialize_flow(self, datapath):
        ...
        if datapath.dp.id == self.tester_dpid:
    ...
    def _test_flow_matching_check(self, pkt):
        """Expects to receive a packet in.
        The tester always has a match any action port:controller
        and the target always has an action port:target_send_port_1
        If the tester or target don't send it, the test fails.
        ...
        self.logger.info(
            Formatter.fmt_exc(
                ...
                keys=['data'],
                pre_msg='Received PacketIn\n'))
        # self.logger.info("dpid=%s : receive_packet[%s]",
        ...
        if msg.datapath.id != pkt_in_src_model.dp.id:
            ...
            diff = self._diff_packets(
                packet.Packet(model_pkt), packet.Packet(msg.data))
            err_msg = 'diff egress/ingress: {}'.format(diff)
        else:
            return TEST_OK

        raise TestFailure(self.state, pkt_type=pkt_type, detail=err_msg)

    def _get_before_and_after_port_stats(self, target_pkt_count,
                                         tester_pkt_count):
        """TODO: Docstring for _get_before_and_after_port_stats.

        :target_pkt_count: target_pk_count obj
        :tester_pkt_count: tester_pkt_count obj
        :returns: dictionary with the fowing keys:

        """
        result = {}

        result['before_target_receive'] = target_pkt_count[0][
            self.target_recv_port]['rx']
        result['before_target_send'] = target_pkt_count[0][
            self.target_send_port_1]['tx']
        result['before_tester_receive'] = tester_pkt_count[0][
            self.tester_recv_port_1]['rx']
        result['before_tester_send'] = tester_pkt_count[0][
            self.tester_send_port]['tx']

        result['after_target_receive'] = target_pkt_count[1][
            self.target_recv_port]['rx']
        result['after_target_send'] = target_pkt_count[1][
            self.target_send_port_1]['tx']
        result['after_tester_receive'] = tester_pkt_count[1][
            self.tester_recv_port_1]['rx']
        result['after_tester_send'] = tester_pkt_count[1][
            self.tester_send_port]['tx']

        return result

    def _test_no_pktin_reason_check(self, test_type, target_pkt_count,
                                    tester_pkt_count):
        res = self._get_before_and_after_port_stats(target_pkt_count,
                                                    tester_pkt_count)

        if res['after_tester_send'] == res['before_tester_send']:
            log_msg = 'no change in tx_packets on tester.'
        elif res['after_target_receive'] == res['before_target_receive']:
            log_msg = 'no change in rx_packets on target.'
        elif test_type == KEY_EGRESS:
            if res['after_target_send'] == res['before_target_send']:
                log_msg = 'no change in tx_packets on target.'
            elif res['after_tester_receive'] == res['before_tester_receive']:
    ...
    def _test_send_barrier(self):
        ...
        assert isinstance(msg,
                          self.tester_sw.dp.ofproto_parser.OFPBarrierReply)

    def _one_time_packet_send(self, pkt):
        self.logger.info(
            Formatter.fmt_exc(
                packet.Packet(pkt[KEY_INGRESS]),
                keys=['data'],
                pre_msg='Sent PacketOut\n'))
    ...
    def _continuous_packet_send(self, pkt):
        ...
        arg = {
            'packet_text': pkt_text,
            'packet_binary': pkt_bin,
            'thread_counter': 0,
            'dot_span':
            int(CONTINUOUS_PROGRESS_SPAN / CONTINUOUS_THREAD_INTVL),
    ...
    def _compare_flow(self, stats1, stats2):
        ...
        attr_list = [
            'cookie', 'priority', 'hard_timeout', 'idle_timeout', 'match'
        ]
        ...
        for rcv_p in rcv_pkt.protocols:
            if not isinstance(rcv_p, six.binary_type):
                ...
                if len(model_protocols) == 1:
                    ...
                    if diff:
                        msg.append('%s(%s)' % (rcv_p.__class__.__name__,
                                               ','.join(diff)))
                else:
                    if (not model_protocols
                            or not str(rcv_p) in str(model_protocols)):
    ...
    @set_ev_cls([
        ofp_event.EventOFPFlowStatsReply, ofp_event.EventOFPTableStatsReply,
        ofp_event.EventOFPPortStatsReply, ofp_event.EventOFPGroupDescStatsReply
    ...
    def stats_reply_handler(self, ev):
        ...
        event_states = {
            ofp_event.EventOFPFlowStatsReply: [
                STATE_FLOW_EXIST_CHK, STATE_THROUGHPUT_FLOW_EXIST_CHK,
                STATE_GET_THROUGHPUT
            ],
            ofp_event.EventOFPPortStatsReply:
            [STATE_TARGET_PKT_COUNT, STATE_TESTER_PKT_COUNT],
        }
        if ofp.OFP_VERSION >= ofproto_v1_2.OFP_VERSION:
    ...
    def barrier_reply_handler(self, ev):
        state_list = [
            STATE_INIT_FLOW, STATE_INIT_THROUGHPUT_FLOW, STATE_INIT_GROUP,
            STATE_FLOW_INSTALL, STATE_THROUGHPUT_FLOW_INSTALL,
            STATE_GROUP_INSTALL, STATE_SEND_BARRIER
        ]
    ...
    @set_ev_cls(ofp_event.EventOFPErrorMsg, [
        handler.HANDSHAKE_DISPATCHER, handler.CONFIG_DISPATCHER,
        handler.MAIN_DISPATCHER
    ])
...
class OpenFlowSw(object):
    ...
    def del_groups(self):
        ...
        mod = parser.OFPGroupMod(
            self.dp, command=ofp.OFPGC_DELETE, type_=0, group_id=ofp.OFPG_ALL)
        return self.send_msg(mod)
    ...
    def send_flow_stats(self):
        ...
        parser = self.dp.ofproto_parser
        if ofp.OFP_VERSION == ofproto_v1_0.OFP_VERSION:
            req = parser.OFPFlowStatsRequest(self.dp, 0, parser.OFPMatch(),
                                             0xff, ofp.OFPP_NONE)
        else:
            req = parser.OFPFlowStatsRequest(self.dp, 0, ofp.OFPTT_ALL,
                                             ofp.OFPP_ANY, ofp.OFPG_ANY, 0, 0,
                                             parser.OFPMatch())
        return self.send_msg(req)
...
class TestSuite(object):
    def pytest_sessionfinish(self):
        self.of_tester._test_end()

    def __init__(self, of_tester, logger):
        """Init.

        """
        self.of_tester = of_tester
        self.logger = logger
        self.collect_only = False

    @pytest.fixture
    def oft(self):
        return self.of_tester


class TestJson(stringify.StringifyMixin):
    """Test Json object include Test objects."""

    def __init__(self, of_tester, json_data, description):
        self.of_tester = of_tester
        self.description = description
        self._normalize_test_json(json_data)
        self.test = Test(json_data)

    def _normalize_test_json(self, val):
        def __replace_port_name(k, v):
            if v[k] == 'target_recv_port':
                v[k] = self.of_tester.target_recv_port
            elif v[k] == 'target_send_port_1':
                v[k] = self.of_tester.target_send_port_1
            elif v[k] == 'target_send_port_2':
                v[k] = self.of_tester.target_send_port_2
            elif v[k] == 'tester_send_port':
                v[k] = self.of_tester.tester_send_port
            elif v[k] == 'tester_recv_port_1':
                v[k] = self.of_tester.tester_recv_port_1
            elif v[k] == 'tester_recv_port_2':
                v[k] = self.of_tester.tester_recv_port_2
...
class Test(stringify.StringifyMixin):
    def __init__(self, test_json):
        ...
        (self.description, self.prerequisite,
    ...
    def _parse_test(cls, buf):
        ...
        for test in buf[KEY_TESTS]:
            ...
            if isinstance(test[KEY_INGRESS], list):
                ...
                test_pkt[KEY_PACKETS] = {
                    ...
                    __test_pkt_from_json(
                        test[KEY_INGRESS][KEY_PACKETS][KEY_DATA]),
                    KEY_DURATION_TIME:
                    test[KEY_INGRESS][KEY_PACKETS].get(KEY_DURATION_TIME,
                                                       DEFAULT_DURATION_TIME),
                    KEY_PKTPS:
                    test[KEY_INGRESS][KEY_PACKETS].get(KEY_PKTPS,
                                                       DEFAULT_PKTPS),
                    'randomize':
                    True in [
                        line.find('randint') != -1
            ...
            if KEY_FLOW_MISS in test:
                test_pkt[KEY_FLOW_MISS] = 0
